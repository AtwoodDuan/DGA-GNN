...
# 数据划分
trn_idx, rst_idx, y_trn, y_rst = train_test_split(range(len(y)), y, stratify=y, train_size=0.8 ,shuffle=True)
val_idx, y_val = rst_idx, y_rst
tst_idx, y_tst = rst_idx, y_rst
...
# elliptic
elliptic_txs_classes  = pd.read_csv('/data1/all_data/elliptic/elliptic_txs_classes.csv')
elliptic_txs_edgelist = pd.read_csv('/data1/all_data/elliptic/elliptic_txs_edgelist.csv')
elliptic_txs_features = pd.read_csv('/data1/all_data/elliptic/elliptic_txs_features.csv', header=None)
# X = elliptic_txs_features.iloc[:,1:167-72].values
# X = pd.DataFrame(X,columns=[f'f{i}' for i in range(167-72-1)])
X = elliptic_txs_features.iloc[:,1:167].values
X = pd.DataFrame(X,columns=[f'f{i}' for i in range(167-1)]).fillna(-1)
elliptic_txs_classes['class'] = elliptic_txs_classes['class'].map({'unknown':-1,'2':0,'1':1})
y = elliptic_txs_classes['class'].values
nname2nidx = dict(zip(elliptic_txs_features[0],elliptic_txs_features.index))
elliptic_txs_classes['txId'] = elliptic_txs_classes['txId'].map(nname2nidx)
elliptic_txs_edgelist['txId1'] = elliptic_txs_edgelist['txId1'].map(nname2nidx)
elliptic_txs_edgelist['txId2'] = elliptic_txs_edgelist['txId2'].map(nname2nidx)
#elliptic_txs_edgelist.columns = ['n0','n1']
labeled = elliptic_txs_classes[elliptic_txs_classes['class']!=-1]
trn_idx, rst_idx, y_trn, y_rst = train_test_split(labeled['txId'].values, labeled['class'], stratify=labeled['class'],
                                                     train_size=0.8, random_state=2,shuffle=True)

elliptic_txs_classes  = pd.read_csv('/data1/all_data/elliptic/elliptic_txs_classes.csv')
elliptic_txs_classes['class'] = elliptic_txs_classes['class'].map({'unknown':0,'2':0,'1':1})
y = elliptic_txs_classes['class'].values
...
# trn_idx = pd.Series(trn_idx).map(nname2nidx)
# rst_idx = pd.Series(rst_idx).map(nname2nidx)
num_nodes = len(y)
val_idx, y_val = rst_idx, y_rst
tst_idx, y_tst = rst_idx, y_rst
...
#构建图
src_nodes = elliptic_txs_edgelist['txId1']
dst_nodes = elliptic_txs_edgelist['txId2']
graph = dgl.graph((src_nodes, dst_nodes), num_nodes=num_nodes)
